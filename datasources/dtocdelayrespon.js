...
// dtocdelayrespon Mongo Model
var mongoose = require('mongoose');
mongoose.Promise = require('bluebird');

var dtocdelayresponSchema = new mongoose.Schema({
  filename: String,
  created_At: {
    type: Date,
    required: true,
...
dtocdelayresponSchema.index({Period: 1}, {unique: true})

dtocdelayresponSchema.statics.findByFilename = function (filename) {
  var dtocdelayresponLoad = this;

  return dtocdelayresponLoad.findOne({
    'filename': filename
  });
};

dtocdelayresponSchema.statics.getAll = function () {
  var dtocdelayresponLoad = this;

  return dtocdelayresponLoad.find({});
};

const mongoModel = mongoose.model('dtocdelayresponLoad', dtocdelayresponSchema);

// dtocdelayrespon Glob Pattern
const globPattern = [
    '../nhs_england/Trust-Type-B-*.xls',
];

// dtocdelayrespon regex
const regex = new RegExp(/Trust-Type-B-.*\.xls/g);

// dtocdelayrespon Data Process function
