...
// ca31subradio Mongo Model
var mongoose = require('mongoose');
mongoose.Promise = require('bluebird');

var ca31subradioSchema = new mongoose.Schema({
  filename: String,
  created_At: {
    type: Date,
    required: true,
...
ca31subradioSchema.statics.findByFilename = function (filename) {
  ...
  return ca31subradioLoad.findOne({
    'filename': filename
  });
};

ca31subradioSchema.statics.getAll = function () {
  var ca31subradioLoad = this;

  return ca31subradioLoad.find({});
};

const mongoModel = mongoose.model('ca31subradioLoad', ca31subradioSchema);
...
// ca31subradio Data Process function
const processData = function (mongoDataRaw) {
    ...
    for (var prop in mongoDataRaw[11][1]) {
    ...
    let dataMapping = mongoDataRaw[11][8];

    formattedMongoData.data = mongoDataRaw[11].slice(9);
