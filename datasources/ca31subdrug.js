...
// ca31subdrug Mongo Model
var mongoose = require('mongoose');
mongoose.Promise = require('bluebird');

var ca31subdrugSchema = new mongoose.Schema({
  filename: String,
  created_At: {
    type: Date,
    required: true,
...
ca31subdrugSchema.statics.findByFilename = function (filename) {
  ...
  return ca31subdrugLoad.findOne({
    'filename': filename
  });
};

ca31subdrugSchema.statics.getAll = function () {
  var ca31subdrugLoad = this;

  return ca31subdrugLoad.find({});
};

const mongoModel = mongoose.model('ca31subdrugLoad', ca31subdrugSchema);

// ca31subdrug Glob Pattern
const globPattern = [
...
// ca31subdrug Data Process function
const processData = function (mongoDataRaw) {
    ...
    for (var prop in mongoDataRaw[10][1]) {
    ...
    let dataMapping = mongoDataRaw[10][8];

    formattedMongoData.data = mongoDataRaw[10].slice(9);
