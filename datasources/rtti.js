...
var rttiSchema = new mongoose.Schema({
  filename: String,
  created_At: {
    type: Date,
    required: true,
...
rttiSchema.statics.findByFilename = function (filename) {
  ...
  return rttiLoad.findOne({
    'filename': filename
  });
};

rttiSchema.statics.getAll = function () {
  var rttiLoad = this;

  return rttiLoad.find({});
};

const mongoModel = mongoose.model('rttiLoad', rttiSchema);
...
const processData = function (mongoDataRaw) {
    ...
    const dataStructure = {
      'Area Team Code': undefined,
       ...
       'Total number of incomplete pathways': undefined,
