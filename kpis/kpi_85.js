...
const transformFunction = function (load) {
    ...
    load.data.forEach(loadDataItem => {
        if (loadDataItem && //check item exists
            ...
            loadDataItem["Occupied - Number of Adult critical care beds"].toString().length > 0 && //check it has field for Value
            loadDataItem["Occupied - Number of Adult critical care beds"].toString() !== '-' &&
            loadDataItem["Occupied - Number of Paediatric intensive care beds"].toString().length > 0 && //check it has field for Value
            loadDataItem["Occupied - Number of Paediatric intensive care beds"].toString() !== '-' &&
